...
import { runFusion, runFusionPanelJudge, resolveFusionConfig, buildOuterModelRequest, } from "./fusion.js";
...
app.post("/v1/chat/completions", async (c) => {
    ...
    const { messages, fusion_config, temperature, max_tokens, stream } = parsed.data;
...
async function handleFusionNonStream(c, data, fc) {
    try {
        ...
        return c.json({
            ...
            fusion_metadata: {
                ...
                panel_responses: result.panelResponses.map((r) => ({
                    model: r.model,
                    error: r.error ?? false,
...
// ---- Streaming fusion ----
async function handleFusionStream(c, data, fc) {
    const messages = data.messages;
    const fusionId = `fusion-${Date.now()}`;
    return streamSSE(c, async (stream) => {
        try {
            // 1. Panel + judge (internal, not streamed)
            const { panelResponses, analysis, judgeRawContent } = await runFusionPanelJudge({
                messages,
                fusionConfig: fc,
            });
            ...
            const outerReq = buildOuterModelRequest(judgeRawContent, "", messages, fc.outer_model, fc.temperature, fc.max_tokens);
            ...
            stream.writeSSE({
                data: JSON.stringify({
                    ...
                    fusion_metadata: {
                        ...
                        judge: fc.judge,
                        web_search: fc.web_search,
                        panel_responses: panelResponses.map((r) => ({
                            model: r.model,
                            error: r.error ?? false,
                            content_length: r.content.length,
                        })),
...
// ---- Proxy to LiteLLM ----
async function proxyToLiteLLM(c, body) {
