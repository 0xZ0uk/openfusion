...
import { JUDGE_SYSTEM_PROMPT, buildJudgeUserMessage, buildOuterSystemPrompt, formatSearchResults, } from "./prompts.js";
 ...
 */
async function maybeSearch(messages, webSearchEnabled) {
    if (!webSearchEnabled || !config.search.enabled)
        return "";
    ...
    if (results.length === 0)
        return "";
    console.log(`[fusion] Web search returned ${results.length} results for: ${query.slice(60)}${query.length > 60 ? "…" : ""}`);
 ...
 */
export async function runFusionPanelJudge(input) {
    ...
    const panelParams = panel.map((model) => ({
        model,
        messages: panelMessages,
        temperature,
        max_tokens,
    }));
    const panelSettled = await Promise.allSettled(panelParams.map((p) => callLiteLLM(p)));
    ...
    const totalPrompt = panelPromptTokens + (judgeResult.usage?.prompt_tokens ?? 0);
    const totalCompletion = panelCompletionTokens + (judgeResult.usage?.completion_tokens ?? 0);
    return {
        panelResponses,
        analysis: analysis,
 ...
 */
export function buildOuterModelRequest(judgeRawContent, searchContext, messages, outerModel, temperature, max_tokens) {
    const outerSystemPrompt = buildOuterSystemPrompt(judgeRawContent, searchContext);
...
export async function runFusion(input) {
    ...
    // Panel + judge
    const { panelResponses, analysis, judgeRawContent, usage } = await runFusionPanelJudge(input);
    // Step 0 shared — run search again for outer model context
    const searchContext = await maybeSearch(messages, fusionConfig.web_search);
    // Step 3: outer model (if configured)
    if (outerModel) {
        const outerReq = buildOuterModelRequest(judgeRawContent, searchContext, messages, outerModel, fusionConfig.temperature, fusionConfig.max_tokens);
 ...
 */
export function resolveFusionConfig(fusionConfig, requestModel) {
    const defaults = config.defaults;
    let panel = fusionConfig?.panel ?? defaults.panel;
    if (panel.length === 0)
        panel = defaults.panel;
    if (panel.length > 8)
        panel = panel.slice(0, 8);
    return {
        panel,
